...
class TgBotNotifier:
    ...
    async def convert_prices(self, flights: list[Flight]) -> list[Flight]:
        ...
        async def convert(flight: Flight) -> Flight:
            flight.price = Decimal(
                round(await currency_converter.convert(flight.price, flight.currency, self.currency))
            )
            if flight.prev_price is not None:
                flight.prev_price = Decimal(
                    round(await currency_converter.convert(flight.prev_price, flight.currency, self.currency))
                )
            return flight
